...
class Database():
    ...
    def display(self):
        ...
        for row in range(self.rowCount):
            for col in range(8):
                self.model.setItem(row,col,QtGui.QStandardItem(self.queryResult[row][col]))
    
    # def getRowCount(self):
    #     try:
    #         self.cursor.execute("SELECT COUNT(*) FROM account.customer")
...
class myWindow(Ui_mainWindow, QtWidgets.QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        self.tableViewResult.setModel(self.db.model)
        
    def query(self):
        self.db.query(self.comboQuery.currentText(), self.textKeyWord.text())
        # self.rowCount = self.db.getRowCount()
        self.db.display()
